...
PIV解析には multi-pass 相互相関法を用い, interrogation window は 128 pixel から 64 pixel を経て 32 pixel へ 3 段階で縮小し, 最終 pass の overlap は 50\% とした(画像対間隔は流量水準ごとに異なり, 後述する). 得られた速度ベクトル場に対しては, 標準偏差フィルタ($6\sigma$)および局所メディアンフィルタにより明らかな誤ベクトルを除去し, 必要最小限の補間を行った(inflow 条件では補間ベクトルの割合は 1--3\% であった). また, port 近傍では反射, 画像歪み, および局所的な三次元性の影響により表面PIVの信頼性が低下する可能性があるため, 指標計算には当該領域を除外した解析領域 $\Omega$ を用いた. マスク範囲は全条件で共通とし, basin 内の大域的な流動構造の比較に主眼を置いた.
...
$I_{\mathrm{circ}}(t)$ &
Eq.~\eqref{eq:Icirc} &
Signed circulation index. Characterizes the sign and strength of the net basin-scale rotation; near zero both for cancelling counter-rotating cells and for irrotational flow. \\
