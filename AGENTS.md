...
- 昇格本文は「問い／手法／主要知見／限界・適用範囲／本研究との接点」の箇条書きで記す（リファレンス文書として一覧性を優先）。

記述の階層（量は周辺度に応じて意図的に変える。全文献にフルメモは付けない）:
- 核心 = フル構造化メモ（上記の箇条書き）。20〜40 本程度を想定。
- 補助 = 論文名見出し + 1〜2 文（本研究との接点を中心に）。
- 周辺 = 標準メモを作らず、各節の「論文横断整理」に 1 行、または原稿執筆時に `\cite` で拾う。
review_master.tex は最終引用リスト（〜50 本想定）の上位集合。read したが引用しない論文も「周辺」として残し、再調査を防ぐ。

節には 2 種類ある:
- 主題節（§1〜§6）= 文献の本体メモを置く。1 文献の本体メモは 1 箇所のみ。
- 索引節（§7 流れの安定性と分岐）= 手法・枠組みの観点から横断的に参照する。本体メモは主題節に置いたままにし、索引節には要約と参照先（［本体は §N］）のみを記す。現象の文脈から切り離さないための措置。将来その節を主戦場とする文献を入手した場合のみ、本体メモを索引節の「論文メモ」に置く。
...
入手できなかった文献（アクセス権なし等）は review_master.tex の「入手メモ」節に記録し、再調査を防ぐ（入手でき次第、該当セクションへ移す）。
